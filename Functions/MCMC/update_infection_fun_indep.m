function [data_struct_augmented_new,ll_household_new,acceptance] = update_infection_fun_indep(theta,data_struct_augmented_old,ll_household_old,ll_household_form,t_i_prop_sd_symp)
    ...
    % Randomly select one symptomatic infected host in every household
    % containing at least one such individual
    
    r1 = rand(sum(symp_in_household),1);
    update_hosts_in_household = ceil(no_symp_in_household(symp_in_household).*r1);
    ...
    r2 = randn(sum(symp_in_household),1);
