function data_struct_augmented = initialise_augmented_data_indep(data_struct_observed)
    ...
    if sum(symp)>0
        ...
        t_i(asymp) = t_s_min + (t_s_max-t_s_min)*rand(sum(asymp),1);
    else
        t_i(asymp) = rand(sum(asymp),1);
    end
    
    t_i = max(min(t_i,t_iR),t_iL); %ensure initial values lie within bounds
